 ...
 *
 * 【繰り返しは行を増やさず、期限だけを進める】
 * 毎週のものを完了にすると、**同じ行の期限が次の週へ動く。**
 * 行を増やさないのは、lib/use-table.ts が select * で全行を端末に写す作りで、
 * 増やすとそのまま端末のキャッシュが太るため(毎日のごみ出しで年365行)。
 * 期限が何周期ぶんも過ぎていたときは、今日を追い越すまでまとめて進む。
 * 「いつ済ませたか」を1件ずつ残したい繰り返しは、家事(chores / chore_log)の担当。
 * 決めた理由は lib/mutations.ts の setTodoDone に書いてある。
 */
