...
| タブ | できること |
|---|---|
| **買い物** | 売り場順・行全体タップ・残り件数・セール枠は別枠・**切らした常備品の札**・買い物をおわると片付く。**オフラインでも動く** |
...
### スマホだけで閉じる道(パソコンが要らない)

inbox の受け渡しは**パソコンが起きている必要がある。**レシートを撮るのは
スマホなのに、記録するにはパソコンの前に戻ることになっていた。
そこでアプリ側に、貼るだけで済む道を2本置いてある。

| やること | 画面 | 作り |
|---|---|---|
| 献立の相談・レシピ生成 | `/recipes/ask` | 在庫入りの相談文を作る([lib/ai-context.ts](lib/ai-context.ts))→ 返事を貼ると登録 |
| レシート・予定・やることの記録 | `/handoff` | 書き方の依頼文を作る([lib/handoff-prompt.ts](lib/handoff-prompt.ts))→ 返ってきた JSON を貼ると記録 |

どちらも**スキルを積んでいないチャット**(スマホの Claude アプリ)で成立する。
アプリはスマホからでも Supabase に届くので、パソコンも追加の課金も要らない。

`/handoff` の依頼文が説明する op は、受け取る側([lib/handoff.ts](lib/handoff.ts))の
`HandoffOp` で型が付いている。**op を足すと、説明を書くまで `tsc` が落ちる。**
頼む側と受け取る側が食い違うと、チャットは正しいつもりで入らない JSON を返し続け、
貼るまで誰にも分からない。

記録ごとの `key`(二重取り込みよけ)は、**無ければ中身から作る**
(`op|正規化した args` の SHA-256。`cowork/jisui/db.py` の `_handoff_record` と同じ形)。
スキルを持たないチャットは鍵を付けないので、作らないと二度貼りが素通りする。

貼る側は**返事をまるごと**受け取る。依頼文はチャットに「説明はコードブロックの外に
書く」と頼んでいるので、返事には必ず前後に文章が付く。`parseHandoff` は
素のまま → コードブロックの中 → いちばん外側の `{ … }` の順に試す。
...
lib/staples.ts          常備品を切らしていないかの判定。
                        同じ規則が scripts/restock-staples.mjs にもある(両方直すこと)
