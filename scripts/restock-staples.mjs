 ...
 *
 * 【アプリも同じ印を見て足すようになった(2026-09)】
 * 在庫画面で数量が0になった/「使い切った」を押した瞬間に、アプリが
 * その場で買い物リストへ足す(lib/inventory-store.ts の restockIfOut)。
 * 見ている印は**こちらと同じ pantry.staple** で、規則を2つに分けていない。
 *
 * それでも二重には足さない。**足す前に「未購入の同じ品」を見る**からで、
 * アプリが先に足していれば、こちらは already() で飛ばす(逆も同じ)。
 * このスクリプトが要らなくなったわけでもない。pantry.stock の
 * 「切れた/切れそう」(在庫として数えていない醤油など)は、
 * 在庫の0では拾えないので、こちらでしか気づけない。
 *
 * 【使い方】
