...
# 届かなかったときに【必ず】付ける案内。
#
# 【403 と判定できたときだけに埋めないこと】
# 以前は 403 の説明文の中にだけ書いていた。理由が違う切れ方をすると案内が
# まるごと消え、素っ気ない通信エラーだけが残る。配る前の試験で見つかった。
# 届かない理由が何であれ、やることは同じなので、ここで常に付ける。
_OFFLINE_TAIL = (
    "\n\n"
    "【読むだけなら、これで足ります】\n"
    f"  {os.path.dirname(INBOX_DIR)}\\いまの状況.md\n"
    "  パソコンの見張りが5分ごとに書き出しています。在庫(冷蔵・氷温・野菜・\n"
    "  冷凍・常温の区画ごと)、これからの献立、買い物リスト、好み・NG、\n"
    "  直近に作ったもの、調理器具、常備品が入っています。\n"
    "  あなたのファイル読み取りの道具で読めます。Supabase は要りません。\n"
    "  中身が古いときは、inbox に 更新して.txt という空ファイルを置いてください。\n"
    "  数秒で書き直されます。置いたら5秒ほど待って読み直すこと。\n"
    "  【「在庫が読めないので推測で提案します」と言わないこと。】読めます。\n"
    "\n"
    "【全部を直接やりたいなら、パソコン上で動かしてください】\n"
    "  ・タスクの「…」(その他のオプション)→「お使いのコンピュータに移動」\n"
    "  ・または設定の「このタスクの実行場所」→「このコンピューター」\n"
    "  移すと「このコンピューターで実行されます」と出ます。\n"
    "  ただし「プロジェクト」を付けたタスクは移せません。フォルダを付けること。\n"
)
...
def _request(method: str, url: str, headers: dict[str, str], body: Any = None) -> Any:
    ...
    try:
        ...
        #
        # 案内は「403 のときだけ」に埋めないこと。理由が何であれ届かないなら
        # やることは同じで、埋めると 403 以外で無言になる。実際にそうなっていた。
        # 常に出るぶんは _OFFLINE_TAIL にまとめてある。
        ...
        raise JisuiOffline(
            f"通信できませんでした: {Jisui._explain_network_error(e.reason)}{hint}{_OFFLINE_TAIL}",
            records,
        ) from e
